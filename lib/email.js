...
/**
 * Send abandonment email to users who completed assessment but didn't pay
 * @param {string} email - User email
 * @param {string} userName - User name
 * @param {string} userId - User ID for the profile link
 * @returns {Promise<{success: boolean}>}
 */
export async function sendAbandonmentEmail(email, userName, userId) {
  const firstName = userName ? userName.split(' ')[0] : 'profesional'
  const profileUrl = `https://carrera.negoia.com/profile?userId=${userId}`
  
  const subject = `Tu análisis de carrera lleva 24h esperándote, ${firstName}`
  
  const html = `
<div style="font-family:-apple-system,'Inter',system-ui,sans-serif;background:#0f172a;padding:32px 16px;">
  <div style="max-width:600px;margin:0 auto;">
    <div style="text-align:center;padding:0 0 24px;">
      <span style="font-size:22px;font-weight:800;color:#f8fafc;">carrera<span style="color:#a5b4fc">.</span>ia</span>
    </div>
    <div style="background:linear-gradient(135deg,#1e293b,#0f172a);border-radius:16px;overflow:hidden;border:1px solid rgba(99,102,241,0.2);">
      <div style="padding:40px 32px;text-align:center;">
        <div style="width:72px;height:72px;background:rgba(99,102,241,0.15);border-radius:50%;margin:0 auto 20px;display:flex;align-items:center;justify-content:center;font-size:32px;">🗺️</div>
        <h1 style="margin:0 0 12px;font-size:26px;color:#f8fafc;font-weight:700;">Tu Mapa de Habilidades<br>está listo</h1>
        <p style="margin:0;color:#94a3b8;font-size:16px;line-height:1.6;">Y tus roles compatibles te están esperando, ${firstName}.</p>
      </div>
      ...
      <div style="padding:0 32px 32px;">
        <div style="background:rgba(99,102,241,0.08);border:1px solid rgba(99,102,241,0.2);border-radius:12px;padding:24px;margin-bottom:24px;">
          <p style="margin:0 0 16px;font-size:15px;color:#f8fafc;line-height:1.7;">
            Completaste tu assessment y ya tenemos tu análisis listo. Encontramos <strong style="color:#6366f1;">roles que encajan con tu perfil</strong> y un mapa completo de tus habilidades.
          </p>
          <p style="margin:0;font-size:15px;color:#f8fafc;line-height:1.7;">
            Pero aún no has visto los resultados completos. <strong>Tu análisis expira en 24h</strong> — después tendrías que repetir el proceso.
          </p>
        </div>
        ...
        <div style="background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.2);border-radius:12px;padding:20px;margin-bottom:24px;">
          <p style="margin:0 0 8px;font-size:13px;color:#10b981;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">💡 Dato</p>
          <p style="margin:0;font-size:14px;color:#94a3b8;line-height:1.6;">
            Más de <strong style="color:#f8fafc;">200 profesionales</strong> ya han descubierto su dirección profesional con Carrera.IA. El 78% encontró roles que no conocía.
          </p>
        </div>

        <div style="text-align:center;">
          <a href="${profileUrl}" style="display:inline-block;background:linear-gradient(135deg,#10b981,#059669);color:#fff;text-decoration:none;padding:18px 48px;border-radius:12px;font-weight:700;font-size:16px;box-shadow:0 4px 14px rgba(16,185,129,0.3);">Ver mis roles compatibles →</a>
        </div>
        
        <p style="margin:24px 0 0;font-size:12px;color:#64748b;text-align:center;">
          ⏰ Tu análisis expira en 24h
        </p>
      </div>
    </div>
    <div style="text-align:center;padding:24px 0 0;">
      <p style="margin:0;font-size:12px;color:#64748b;">carrera.negoia.com · Responde este email si tienes preguntas</p>
    </div>
  </div>
</div>`
